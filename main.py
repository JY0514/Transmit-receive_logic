...
def recep():
    ...
    for row in result2:
        ...
        update group_all 
        set end_count  = '0'
        where group_all.rider_id = '{rider_id}'  """
        ...
        sql4 = f"""
                               UPDATE group_all+
                               SET group_count =  group_count + IFNULL((start_count = end_count), 0)
        ...
        for row in resultu:
            ...
            print(resultu_value)
            ql4_update = f"""
                      update group_all set end_count = {resultu_value}
                    """
            cursor.execute(ql4_update)
